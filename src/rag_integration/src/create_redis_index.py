...
# --- Configuration ---
# File paths
DATA_FILE = "src/json/renata_data.json"
...
def create_redis_index():
    ...
    loader = JSONLoader(file_path=DATA_FILE, jq_schema=jq_schema, text_content=True)
    ...
    try:
        ...
        logger.error(
            "Make sure 'jq' is installed (pip install jq). On Windows, you might need Visual Studio C++ Build Tools."
        )
        return
    ...
    logger.info(
        f"🧠 Initializing SentenceTransformerEmbeddings with model: {EMBED_MODEL}..."
    )
