...
COMPANY_DATA_FILE = "src/json/renata_data.json"
INDEX_FILE_PATH = "src/rag_integration/rag_data/company_rag_index.bin"
CORPUS_FILE_PATH = "src/rag_integration/rag_data/company_corpus_chunks.npy"
# The temporary file that holds the plain text to be indexed
KNOWLEDGE_BASE_TEXT_FILE = "src/Audio_transcripts/knowledge_base.txt"
device = "cpu"
...
def load_company_data():
    ...
    if not os.path.exists(file_path):
        ...
        raise FileNotFoundError(
            f"❌ Required file {COMPANY_DATA_FILE} is missing. Please create it."
        )
...
def create_knowledge_base_text(data: dict) -> list[str]:
    ...
    mission = data.get(
        "mission",
        "connecting the physical and digital world to unlock new operational efficiencies.",
    )
...
def create_faiss_index(corpus: list[str]):
    ...
    corpus_embeddings = embedder.encode(
        corpus, convert_to_tensor=False, show_progress_bar=True
    )
    ...
    logger.info(
        f"Raw corpus chunks saved to {CORPUS_FILE_PATH} (Total {len(corpus)} chunks)"
    )
