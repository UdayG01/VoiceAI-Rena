...
def classify_context_type(json_path: str) -> str:
    ...
    elif any(x in path_lower for x in ["organization", "founder", "company_name", "location", "team"]):
        return "company_info"
    elif any(x in path_lower for x in ["market_presence", "customer", "industry", "clients"]):
        return "market_info"
    elif any(x in path_lower for x in ["solution", "capabilities", "production", "quality"]):
...
def load_company_data() -> dict:
    if not os.path.exists(COMPANY_DATA_FILE):
        ...
        logger.warning(f"File {COMPANY_DATA_FILE} not found. Please ensure the file exists.")
        return {}
    ...
    # 2. Handle Lists
        ...
        # Case B: List of Objects (e.g., Emission Scopes, Case Studies)
            ...
            for idx, item in enumerate(data):
                ...
                chunks.extend(
                    extract_contextual_chunks(item, new_path, new_parent, source, min_length)
                )
